...
def get_emotion_from_image(model_for_prediction, img, x: int, y: int, w: int, h: int):
    ...
    face_frame = (torch.from_numpy(face_frame) / 255).reshape(1, 1, 48, 48)

    prediction = model_for_prediction(face_frame)
    ...
    prediction = emotions[int(torch.max(prediction, 1).indices[0])]
